...
namespace Detail
{
	static constexpr ld_t _vectorcall sqrtNewtonRaphson(ld_t x, ld_t curr, ld_t prev) noexcept
	{
		return curr == prev ? curr : sqrtNewtonRaphson(x, .5l * (curr + x / curr), curr);
	}

	static inline ld_t constexpr __vectorcall sqrt(ld_t x) noexcept
	{
		return (x >= 0.l && x < std::numeric_limits<ld_t>::infinity() ? Detail::sqrtNewtonRaphson(x, x, 0.l) : std::numeric_limits<ld_t>::quiet_NaN());
	}

	static inline ld_t constexpr __vectorcall sqr(ld_t x) noexcept
	{
		return (x * x);
	}
